...
class ReservarCitaServiceTest {
    ...
    @Test
    void reservaExitosaRegistraCitaYNotificaPorWhatsApp() {
        ...
        when(notificationPort.enviarConfirmacion(any(Cita.class), eq(paciente.getNumeroWhatsApp())))
                .thenReturn(NotificationPort.NotificationResult.exitosa());
        ...
        assertThat(cita.isNotificacionEnviada()).isTrue();
        assertThat(cita.getFecha()).isEqualTo(franjaDisponible.getFecha());
        assertThat(cita.getHoraInicio()).isEqualTo(franjaDisponible.getHoraInicio());
        verify(franjaHorariaRepositoryPort).marcarComoOcupada(franjaDisponible);
        verify(notificationPort).enviarConfirmacion(any(Cita.class), eq(paciente.getNumeroWhatsApp()));
    }
}
